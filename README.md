뷰 서버사이드 랜더링을 구현할 때 가장 많이 쓰는 프레임워크
SEO 전달
...
또한 beforeCreate나 created에서  window, document같은 브라우저 객체에 접근할 수 없다.
...
csr - html 페이지가 하나로 인식되므로 url 바뀌어도 오픈태그가 변하지 않는다.
ssr - 페이지별로 오픈태그를 설정 할 수 있어 검색엔진 최적화(SEO)에 효과적이다.
...
#설명
.nuxt : 페이지 빌드된 내용
pages : 해당폴더 안에 파일을 생성하면 그 파일기반으로 라우팅이 이루어 진다.(페이지 기반 라우터 자동 생성)
static : 정적인 파일을 넣어주면 된다.(파비콘이나 로봇파일)
store : 스토어작성을 하면 된다.
...
https://imkh.dev/vue-alias-path/  참고
